...
func (cpu *CPU_struct) op_0x2f() {
	...
	cpu.Registers.F = cpu.Registers.F | 0x60
	cpu.Cycle += 4
}
...
func (cpu *CPU_struct) op_0xfe(n uint8) {
	var cp int = int(cpu.Registers.A) - int(n)
	cpu.Registers.F = 0
	cpu.Registers.F = cpu.Registers.F | 0x40
	...
	// check
	if (int(cpu.Registers.A) & 0x10) > (cp & 0x10) {
		cpu.Registers.F = cpu.Registers.F | 0x20
	}

	if cp < 0 {
		cpu.Registers.F = cpu.Registers.F | 0x10
	}
...
func (cpu *CPU_struct) op_0x05() {
	...
	//  TODO: optimize
	// if (((cpu.Registers.B + 1) & 0xF) - (1 & 0xF)) < 0 {
	if cpu.Registers.B&0x10 > (cpu.Registers.B+1)&0x10 {
		cpu.Registers.F = cpu.Registers.F | 0x20
	}
...
func (cpu *CPU_struct) op_0x15() {
	...
	//  TODO: optimize
	// if (((cpu.Registers.D + 1) & 0xF) - (1 & 0xF)) < 0 {
	if cpu.Registers.D&0x10 > (cpu.Registers.D+1)&0x10 {
		cpu.Registers.F = cpu.Registers.F | 0x20
	}
...
func (cpu *CPU_struct) op_0x1d() {
	...
	//  TODO: optimize
	// if (((cpu.Registers.E + 1) & 0xF) - (1 & 0xF)) < 0 {
	if cpu.Registers.E&0x10 > (cpu.Registers.E+1)&0x10 {
		cpu.Registers.F = cpu.Registers.F | 0x20
	}
...
func (cpu *CPU_struct) op_0x3d() {
	...
	//  TODO: optimize
	// if (((cpu.Registers.A + 1) & 0xF) - (1 & 0xF)) < 0 {
	if cpu.Registers.A&0x10 > (cpu.Registers.A+1)&0x10 {
		cpu.Registers.F = cpu.Registers.F | 0x20
	}
...
func (cpu *CPU_struct) op_0x0d() {
	...
	//  TODO: optimize
	// if (((cpu.Registers.C + 1) & 0xF) - (1 & 0xF)) < 0 {
	if cpu.Registers.C&0x10 > (cpu.Registers.C+1)&0x10 {
		cpu.Registers.F = cpu.Registers.F | 0x20
	}
...
func (cpu *CPU_struct) op_0x04() {
	...
	// TODO: optimize
	if cpu.Registers.B&0x08 < (cpu.Registers.B-1)&0x08 {
		cpu.Registers.F = cpu.Registers.F | 0x20
	}
...
func (cpu *CPU_struct) op_0x0c() {
	...
	// TODO: optimize
	if cpu.Registers.C&0x08 < (cpu.Registers.C-1)&0x08 {
		cpu.Registers.F = cpu.Registers.F | 0x20
	}
...
func (cpu *CPU_struct) op_0x24() {
	...
	// TODO: optimize
	if cpu.Registers.H&0x08 < (cpu.Registers.H-1)&0x08 {
		cpu.Registers.F = cpu.Registers.F | 0x20
	}
...
func (cpu *CPU_struct) op_cb_0x11() {
	var carry uint8 = cpu.Registers.F & 0x10
	cpu.Registers.F = 0x00
	cpu.Registers.F = cpu.Registers.F | ((cpu.Registers.C & 0x80) >> 3)
...
func (cpu *CPU_struct) op_cb_0x07() {
	var carry uint8 = cpu.Registers.F & 0x10
	cpu.Registers.F = 0x00
	cpu.Registers.F = cpu.Registers.F | ((cpu.Registers.A & 0x80) >> 3)
...
func (cpu *CPU_struct) op_cb_0x7c() {
	cpu.Registers.F = cpu.Registers.F | 0x20                              //  Set H
	cpu.Registers.F = cpu.Registers.F & 0x30                              //  Reset N and Z
	cpu.Registers.F = cpu.Registers.F | ((cpu.Registers.H & 0x80) ^ 0x80) //  Set Z if bit is 0
	cpu.Cycle += 8
}
...
func (cpu *CPU_struct) op_0x90() {
	...
	//  TODO: optimize
	if cpu.Registers.A&0x10 > (cpu.Registers.A+cpu.Registers.B)&0x10 {
		cpu.Registers.F = cpu.Registers.F | 0x20
	}
...
func (cpu *CPU_struct) op_0x95() {
	...
	//  TODO: optimize
	if cpu.Registers.A&0x10 > (cpu.Registers.A+cpu.Registers.L)&0x10 {
		cpu.Registers.F = cpu.Registers.F | 0x20
	}
...
func (cpu *CPU_struct) op_0xc5() {
	cpu.MMU.WriteWord(cpu.Registers.SP-2, (uint16(cpu.Registers.C)<<8)|uint16(cpu.Registers.B))
	cpu.Registers.SP -= 2
...
func (cpu *CPU_struct) op_0x13() {
	...
	// TODO: optimize, add missing flag
...
func (cpu *CPU_struct) op_0x86() {
	...
	// TODO: optimize, add missing flag
	if cpu.Registers.A&0x08 < (cpu.Registers.A-1)&0x08 {
		cpu.Registers.F = cpu.Registers.F | 0x20
	}
